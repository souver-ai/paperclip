...
export const issues = pgTable(
  "issues",
  {
    ...
    surfaces: jsonb("surfaces").$type<string[]>().notNull().default(sql`'[]'::jsonb`),
    ...
    benjaminRequired: boolean("benjamin_required").notNull().default(false),
    autoMergeEligible: boolean("auto_merge_eligible").notNull().default(false),
    repoLockId: uuid("repo_lock_id"),
  ...
  (table) => ({
    ...
    categoryIdx: index("issues_company_category_idx").on(table.companyId, table.category),
    surfacesIdx: index("issues_surfaces_gin_idx").using("gin", table.surfaces),
    deliveryStateIdx: index("issues_company_delivery_state_idx").on(table.companyId, table.deliveryState),
    blockerTypeIdx: index("issues_company_blocker_type_idx").on(table.companyId, table.blockerType),
    benjaminRequiredIdx: index("issues_company_benjamin_required_idx").on(table.companyId, table.benjaminRequired),
    repoLockIdx: index("issues_company_repo_lock_idx").on(table.companyId, table.repoLockId),
